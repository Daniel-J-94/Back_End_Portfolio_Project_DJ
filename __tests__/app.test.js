...
describe("Delete Comment", () => {
  it("204 should delete a comment with the given comment_id param, should respond with no content", () => {
    return request(app)
      ...
      .expect(204)
      .then((resultResponse) => {
        const resultResponseBody = resultResponse.noContent;
        expect(resultResponseBody).toBe(true);
      });
  });
  it("404 should respond with an error message if given a comment_id that does not exist", () => {
    return request(app)
  ...
  it("400 should respond with an error message if given a comment_id that is an incorrect type", () => {
    return request(app)
      .delete("/api/comments/CatDog")
      .expect(400)
      .then((resultResponse) => {
        const resultResponseBody = resultResponse.body;
        expect(resultResponseBody).toEqual({
